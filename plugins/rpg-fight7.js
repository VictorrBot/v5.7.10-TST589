let handler = async (m, { conn, usedPrefix, participants }) => {
  ...
  m.reply(`*THE FIGHT ON THE MOON ⚔️*\n\nKamu *Level ${global.db.data.users[m.sender].level}* Menantang *'@' +${conn.getName(lawan)} Level ${global.db.data.users[lawan].level}* Untuk Bertarung Di Bulan Dan Sedang Dalam Pertarungan.\n\n*_Tunggu ${lamaPertarungan} Menit Lagi Untuk Melihat Hasil._*`)
...
handler.help = ['fight-moon']
handler.tags = ['rpg']
handler.command = /^(fight-moon(ing)?)$/i
handler.limit = 7
handler.group = true
handler.level = 599
