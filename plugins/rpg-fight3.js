let handler = async (m, { conn, usedPrefix, participants }) => {
  ...
  m.reply(`*FIGHT IN THE DESERT ⚔️*\n\nKamu *Level ${global.db.data.users[m.sender].level}* Menantang *'@' +${conn.getName(lawan)} Level ${global.db.data.users[lawan].level}* Untuk Bertarung Di Gurun Dan Sedang Dalam Pertarungan.\n\n*_Tunggu ${lamaPertarungan} Menit Lagi Untuk Melihat Hasil._*`)
  ...
  if (pointPemain > pointLawan) {
    let hadiah = (pointPemain - pointLawan) * 15000
    ...
    let denda = (pointLawan - pointPemain) * 30000
...
handler.help = ['fight-desert']
handler.tags = ['rpg']
handler.command = /^(fight-desert(ing)?)$/i
handler.limit = 3
handler.level = 199
handler.group = true
