let handler = async (m, { conn, usedPrefix, participants }) => {
  ...
  m.reply(`*THE BATTLE IN THE ACADEMY ⚔️*\n\nKamu *Level ${global.db.data.users[m.sender].level}* Menantang *'@' +${conn.getName(lawan)} Level ${global.db.data.users[lawan].level}* Untuk Bertarung Di Akademi Dan Sedang Dalam Pertarungan.\n\n*_Tunggu ${lamaPertarungan} Menit Lagi Untuk Melihat Hasil._*`)
...
handler.help = ['fight-academy']
handler.tags = ['rpg']
handler.command = /^(fight-academy(ing)?)$/i
handler.limit = true
handler.group = true
