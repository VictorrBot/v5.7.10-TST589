...
if (!effect) throw `*MAKER LOGO TEKS*\n\n*${usedPrefix + command} efek teks*\nExample:\n *${usedPrefix + command} American-flag-3D Loli*\n\nGunakan Ini Jika Butuh 2 Kalimat\n*${usedPrefix + command} efek teks1|teks2*\nExample:\n*${usedPrefix + command} Wolf-Logo-Galaxy felicia|bot*\n\n*Daftar Logo*\n\n🤍 ${usedPrefix + command} ` + effects.map(v => v.title).join(`\n${usedPrefix + command} `)
...
if (typeof res == 'number') throw res == -1 ? `${mg} *Logo ${effect} Tidak Ditemukan Dalam Daftar Logo*` : `*Gunakan Perintah Sebagai Berikut*\n\n${usedPrefix + command} ${effect} ${new Array(res).fill('texto').map((v, i) => v + (i ? i + 1 : '')).join('|')}`
let result = await axios.get(res, {
responseType: 'arraybuffer'
})
await conn.sendFile(m.chat, result.data, 'Error.jpg', `*Sukses Membuat ${effect}*\n*_${wm}_*`, m)
}
handler.help = ['logo']
handler.tags = ['nulis']
