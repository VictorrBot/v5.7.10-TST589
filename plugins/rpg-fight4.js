let handler = async (m, { conn, usedPrefix, participants }) => {
  ...
  m.reply(`*FOURTH LEVEL BATTLE ⚔️*\n\nKamu *Level ${global.db.data.users[m.sender].level}* Menantang *'@' +${conn.getName(lawan)} Level ${global.db.data.users[lawan].level}* Untuk Bertarung Di Kutub Dan Sedang Dalam Pertarungan.\n\n*_Tunggu ${lamaPertarungan} Menit Lagi Untuk Melihat Hasil._*`)
  ...
  if (pointPemain > pointLawan) {
    ...
    let denda = (pointLawan - pointPemain) * 40000
...
handler.help = ['fight-polar']
handler.tags = ['rpg']
handler.command = /^(fight-polar(ing)?)$/i
handler.limit = 4
handler.group = true
handler.level = 299
