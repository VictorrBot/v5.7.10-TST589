...
let handler = async (m, { conn, usedPrefix, text }) => {
	    let time = global.db.data.users[m.sender].lastmulung + 3600000
  if (new Date - global.db.data.users[m.sender].lastmulung< 3600000) throw `*Anda Sudah Mulung, Tunggu Sekitar ${msToTime(time - new Date())} Lagi*`
  ...
  m.reply(`Selamat kamu mendapatkan : \n💰+${botolnye} Botol\📦n+${kardusnye} Kardus\n✨+${kalengnye} Exp`)
  setTimeout(() => {
					conn.reply(m.chat, `*Waktunya Mulung Lagi*`, m)
					}, timeout)
}
handler.help = ['mulung']
handler.tags = ['rpg']
