let handler = async (m, { conn, usedPrefix, participants }) => {
  ...
  m.reply(`*THE FIGHT IN THE TRENCH ⚔️*\n\nKamu *Level ${global.db.data.users[m.sender].level}* Menantang *'@' +${conn.getName(lawan)} Level ${global.db.data.users[lawan].level}* Untuk Bertarung Di Palung Dan Sedang Dalam Pertarungan.\n\n*_Tunggu ${lamaPertarungan} Menit Lagi Untuk Melihat Hasil._*`)
  ...
  if (pointPemain > pointLawan) {
    let hadiah = (pointPemain - pointLawan) * 30000
    ...
    let denda = (pointLawan - pointPemain) * 60000
...
handler.help = ['fight-trough']
handler.tags = ['rpg']
handler.command = /^(fight-trough(ing)?)$/i
handler.limit = 6
handler.group = true
handler.level = 499
