let handler = async (m, { conn, usedPrefix, participants }) => {
  ...
  m.reply(`*THE FIGHT IN HELL ⚔️*\n\nKamu *Level ${global.db.data.users[m.sender].level}* Menantang *'@' +${conn.getName(lawan)} Level ${global.db.data.users[lawan].level}* Untuk Bertarung Di Neraka Dan Sedang Dalam Pertarungan.\n\n*_Tunggu ${lamaPertarungan} Menit Lagi Untuk Melihat Hasil._*`)
...
handler.help = ['fight-hell']
handler.tags = ['rpg']
handler.command = /^(fight-hell(ing)?)$/i
handler.limit = 10
handler.level = 899
handler.group = true
