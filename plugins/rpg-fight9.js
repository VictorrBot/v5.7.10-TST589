let handler = async (m, { conn, usedPrefix, participants }) => {
  ...
  m.reply(`*THE BATTLE IN THE NGL ⚔️*\n\nKamu *Level ${global.db.data.users[m.sender].level}* Menantang *'@' +${conn.getName(lawan)} Level ${global.db.data.users[lawan].level}* Untuk Bertarung Di NGL Dan Sedang Dalam Pertarungan.\n\n*_Tunggu ${lamaPertarungan} Menit Lagi Untuk Melihat Hasil._*`)
  ...
  if (pointPemain > pointLawan) {
    let hadiah = (pointPemain - pointLawan) * 45000
    ...
    let denda = (pointLawan - pointPemain) * 90000
...
handler.help = ['fight-ngl']
handler.tags = ['rpg']
handler.command = /^(fight-ngl(ing)?)$/i
handler.limit = 9
handler.level = 799
handler.group = true
