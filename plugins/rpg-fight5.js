let handler = async (m, { conn, usedPrefix, participants }) => {
  ...
  m.reply(`*THE FIGHT AT THE SEA ⚔️*\n\nKamu *Level ${global.db.data.users[m.sender].level}* Menantang *'@' +${conn.getName(lawan)} Level ${global.db.data.users[lawan].level}* Untuk Bertarung Di Laut Dan Sedang Dalam Pertarungan.\n\n*_Tunggu ${lamaPertarungan} Menit Lagi Untuk Melihat Hasil._*`)
  ...
  if (pointPemain > pointLawan) {
    let hadiah = (pointPemain - pointLawan) * 25000
...
handler.help = ['fight-sea']
handler.tags = ['rpg']
handler.command = /^(fight-sea(ing)?)$/i
handler.limit = 5
handler.group = true
handler.level = 399
