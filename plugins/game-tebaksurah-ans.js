...
export async function before(m) {
    ...
    if (!(id in this.tebaksurah))
        return conn.sendButton(m.chat, '*_Soal Itu Telah Berakhir_*', author, null, buttontebaksurah, m)
    if (m.quoted.id == this.tebaksurah[id][0].id) {
        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text)
        if (isSurrender) {
            clearTimeout(this.tebaksurah[id][3])
            delete this.tebaksurah[id]
            return conn.sendButton(m.chat, '*Kok Menyerah?*', author, null, buttontebaksurah, m)
        }
