let handler = async (m, { conn, usedPrefix, participants }) => {
  ...
  m.reply(`*THE BATTLE IN TOKYO3 ⚔️*\n\nKamu *Level ${global.db.data.users[m.sender].level}* Menantang *'@' +${conn.getName(lawan)} Level ${global.db.data.users[lawan].level}* Untuk Bertarung Di Tokyo3 Dan Sedang Dalam Pertarungan.\n\n*_Tunggu ${lamaPertarungan} Menit Lagi Untuk Melihat Hasil._*`)
  ...
  if (pointPemain > pointLawan) {
    let hadiah = (pointPemain - pointLawan) * 40000
    ...
    let denda = (pointLawan - pointPemain) * 80000
...
handler.help = ['fight-tokyo3']
handler.tags = ['rpg']
handler.command = /^(fight-tokyo3(ing)?)$/i
handler.limit = 8
handler.level = 699
handler.group = true
