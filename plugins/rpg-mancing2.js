...
let handler = async (m, { usedPrefix }) => {
    ...
    if (user.fishingrod < 2) return m.reply(`
*Dibutuhkan Level 2 Pancingan Untuk Memancing*
Beli Pancingan Di: *${usedPrefix}craft*
`.trim())
    if (user.skillfishing < 2) return m.reply(`
*Dibutuhkan Level 2 Fishing Ability Untuk Memancing*
...
`.trim())
    ...
    let text = `*_Anda Telah Memancing Di Danau Taupo, Selandia Baru 🇳🇿_*`
...
handler.help = ['mancing2']
handler.tags = ['rpg']
handler.command = /^(mancing2|(ber)?petualang(ang)?)$/i
